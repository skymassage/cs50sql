-- A column can be specified the data type while creating a table. However, columns in SQLite don't always store one particular data type. 
...
-- Consider a column with a type affinity for Integers: If we try to insert a text "25" into this column, it will be converted into an integer data type.
...
-- Note that SQLite don't has a type affinity for Boolean, but other DBMS's might have. A workaround could be to use 0 or 1 integer values to represent booleans.
-- If we don't specify a type affinity of a column in SQLite, the default type affinity is numeric, so the column would get assigned the numeric type affinity.
